...
for(const status of [400,404]){lookupOverride={status,body:{success:false,error:'Billing not found'}};providerCalls=[];assert.equal((await checkout(request({},auth))).status,200);assert.equal(providerCalls.filter(c=>c.url.endsWith('/checkouts/create')).length,1);}ok('documented 404 and observed 400 missing checkout both create first purchase');
for(const status of [400,401,403,429,500]){lookupOverride={status,body:{success:false,error:'Other provider error'}};providerCalls=[];assert.equal((await checkout(request({},auth))).status,502);assert(!providerCalls.some(c=>c.url.endsWith('/checkouts/create')));}lookupOverride=null;ok('all other lookup failures stop instead of risking duplicate checkout');
