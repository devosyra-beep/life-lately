/* v29 service-worker unit tests: an in-memory Cache adapter, no real installation. */
...
const current='life-lately-v25-landing-20260919';
const caches={open:async()=>cache,keys:async()=>['life-lately-v24-cofrinhos-20260919','unrelated-cache',current],delete:async k=>{deleted.push(k);return true;}};
...
(async()=>{
 ...
 handlers.activate({waitUntil:p=>pending=p});await pending;assert.deepEqual(deleted,['life-lately-v24-cofrinhos-20260919','life-lately-v25-landing-20260919']);assert.equal(claimed,1);ok('Só os caches antigos do Life Lately são removidos');
 ...
 assert(await request('/assets/landing/landing.css','same-origin'));assert(await request('/assets/landing/hero-night-480.webp','same-origin'));ok('CSS e fotografias da landing têm fallback offline');
