/* v31 service-worker unit tests: an in-memory Cache adapter, no real installation. */
...
const current='life-lately-v31-logo-20260919';
const caches={open:async()=>cache,keys:async()=>['life-lately-v24-cofrinhos-20260919','life-lately-v25-landing-20260919','life-lately-v29-nav-20260919','life-lately-v30-nav-20260919','unrelated-cache',current],delete:async k=>{deleted.push(k);return true;}};
...
(async()=>{
 ...
 handlers.activate({waitUntil:p=>pending=p});await pending;assert.deepEqual(deleted,['life-lately-v24-cofrinhos-20260919','life-lately-v25-landing-20260919','life-lately-v29-nav-20260919','life-lately-v30-nav-20260919']);assert.equal(claimed,1);ok('Só os caches antigos do Life Lately são removidos');
 ...
 const before=calls;const core=await request('/core.js','same-origin');assert(core);assert.equal(calls,before);ok('Engine e app assets são carregados da versão em cache');
 assert(await request('/assets/landing/landing.css','same-origin'));assert(await request('/assets/landing/life-lately-sunset.jpg','same-origin'));assert(await request('/assets/brand/life-lately-logo.png','same-origin'));ok('CSS, fotografia e logo têm fallback offline');
 ...
 const app=fs.readFileSync(path.join(dir,'app/index.html'),'utf8');assert(app.includes('src="../core.js"'));assert(app.includes('href="../manifest.webmanifest"'));assert(!app.includes('assets/landing/landing.css'));ok('HTML de login usa recursos do app, sem misturar CSS da landing');
